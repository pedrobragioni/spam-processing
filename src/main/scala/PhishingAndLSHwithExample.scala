...
object PhishingAndLSHwithExample {
  ...
  def main(args: Array[String]) {
    ...
    val messages = hf.map {
      ...
      case _ => (new Vector[String](), new Vector[String](), "", "", "na")
    }.filter(x => x._5 != "na").zipWithIndex
    ...
    val clusters_content = clusters.map(x => (x._2(1), x._1)).join(messages.map(x => (x._2, (x._1._1)))).map(x => (x._2._1, x._2._2))
    ...
    clusters_content.saveAsTextFile ("hdfs://master:8022/" + outputFile + ".cluster_msgs." + System.currentTimeMillis)
