  ...
  def main(args: Array[String]) {
    ...
    val shingles = hf.map {
      case (_, mail) if mail.From != "Invalid" => 
        getContent(mail.Content, mail.Parts_Content_Type)
      case _ => Set.empty[String]
    }.zipWithIndex.map(x => x.swap).cache()

    // Return all distinct shingles present in messages 
    val shinglesVocab = shingles.flatMap(x => x._2.iterator)
      .distinct
      .zipWithIndex
    ...
    val vectorCluster = bands.filter(x => x._2.size >= minClusterSize).map(x => x._2.toList.sorted).distinct().zipWithIndex().map(x => x._1.map(y => (y.asInstanceOf[Long], x._2))).flatMap(x => x.grouped(1)).map(x => x(0))
    val clusters = sparseShingles.join(vectorCluster).map(x => (x._2._2, x._2._1)).groupByKey()
    val scores = clusters.map(row => (row._1, jaccard(row._2.toList)))
    
    val outputFile = inputFile.split ("/").last
    shingles.saveAsTextFile ("hdfs://master:8022/" + outputFile + ".cts." + System.currentTimeMillis)
